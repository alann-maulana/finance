...
function CashOutContent() {
  ...
  const fetchFirstPage = useCallback(async () => {
    ...
    setListError(null);
    try {
      const page = await getCashOutTransactions(vendorId, period, PAGE_SIZE);
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const page = await getCashOutTransactions(vendorId, period, PAGE_SIZE, cursor ?? undefined);
      setTransactions((prev) => [...prev, ...page.transactions]);
      setCursor(page.cursor);
      setHasMore(page.hasMore);
    } catch (err) {
      console.error('[CashOut] loadMore error:', err);
    } finally {
  ...
  return (
    <Box sx={{ px: { xs: 2, sm: 3 }, py: 3, maxWidth: 600, mx: 'auto', pb: '80px' }}>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent sx={{ pb: '16px !important' }}>
          ...
          <Box sx={{ mt: 1.5 }}>
            <Chip
              size="small"
              icon={<TrendingDownRoundedIcon sx={{ fontSize: '14px !important', color: '#F87171 !important' }} />}
      ...
      <Card>
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
            <TrendingDownRoundedIcon sx={{ fontSize: 18, color: '#F87171' }} />
          ...
          {loading && (
            <Box>
              {[...Array(5)].map((_, i) => (
                <Box key={i}>
                  <TransactionSkeleton />
                  {i < 4 && <Divider sx={{ borderColor: 'rgba(248,113,113,0.08)' }} />}
                </Box>
              ))}
            </Box>
          )}
          ...
          {!loading && transactions.length === 0 && !listError && (
            ...
            >
              ...
              >
                <CallMadeRoundedIcon sx={{ fontSize: 36, color: 'rgba(248,113,113,0.5)' }} />
              </Box>
              ...
              >
                Tekan tombol + untuk mencatat dana keluar pertama di periode ini.
              </Typography>
            </Box>
          )}
          ...
          {loadingMore && (
            <Box sx={{ display: 'flex', justifyContent: 'center', py: 2 }}>
              <CircularProgress size={24} sx={{ color: '#F87171' }} />
            </Box>
          )}
